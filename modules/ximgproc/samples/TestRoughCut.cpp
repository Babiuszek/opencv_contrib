...
void RCApplication::performRoughCut(double epsilon, int windowSize)
{
	...
	roughCut( *images[0], *images[2], *filters, *segmentation, epsilon, windowSize);
...
{
	/*
	//C:/Users/Mateusz/Desktop/Fortress/Entrance and 1st Level/FirstLevelSketches.png
	//./bin/databases/shapes ./bin/databases/textures
    if( argc > 1 )
    {
		...
		namedWindow( imageWinName, WINDOW_AUTOSIZE );
		imshow(imageWinName, image);
    ...
    for(;;)
    {
        ...
        case 'g':
            ...
            break;
		case 's':
			cout << "Enter shapes path: ";
			path = getFilePath();
			cstr = new char[path.length() + 1];
			strcpy(cstr, path.c_str());
			cout << "Loading files...\n";
			app.setGeneratorShapes( cstr );
			break;
		case 't':
			cout << "Enter textres path: ";
			path = getFilePath();
			cstr = new char[path.length() + 1];
			strcpy(cstr, path.c_str());
			cout << "Loading files...\n";
			app.setGeneratorTextures( cstr );
			break;
		case 'r':
			app.reset();
		case '1':
